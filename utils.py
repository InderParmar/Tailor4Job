...
def process_files(files, api_key, model_list, mode, token_usage):
    """
    Process input files using the Groq SDK (for now) with support for multiple models.
    Return the tailored content and token info for each model.
    """
    # Results dictionary to hold the output for each model
    results = {}
    ...
    # Loop through each model in the model_list and process the content
    for model in model_list:
        # Initialize the Groq client for each model (since we are currently assuming Groq for all)
            ...
            2. **Analysis Results**:
               - Compare the Resume to the Job Description:
                ...
                - What are the candidate's major mistakes that are being made while formatting the Resume? If there are any major mistakes, mention them here,
                    otherwise just ignore this section and jump to the Next section.
        ...
        # Store the tailored content and token info for this model
        results[model] = (tailored_content, token_info)
